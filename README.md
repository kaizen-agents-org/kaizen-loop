...
- Builder providers edit the isolated worktree but do not run `commands.setup` or
  ...
  credential-free verification phase. Local runs return failures for a repair attempt
  when the Builder produced edits and fail closed when it did not; the reusable Actions
  workflow fails its verification job closed.
