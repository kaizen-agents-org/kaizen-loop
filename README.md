...
- `guardian.command` runs the vendored `pr-guardian` workflow after PR creation; `guardian.mode: async` persists resumable jobs under `~/.kaizen/projects/<slug>/guardian/jobs/`, while unresolved review threads keep the guardian loop running up to `guardian.maxAttempts`. In repositories that require conversation resolution, outdated unresolved threads still block merging until they are resolved.
