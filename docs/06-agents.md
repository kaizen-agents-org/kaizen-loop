...
5. この provider job では repository setup / verification command を実行しない。変更を未コミットのまま残し、資格情報を持たない後段の Kaizen Loop 検証フェーズへ渡す。Kaizen Loop が `{commands.verify}` をすべて実行する。ローカル実行は Builder の変更がある場合だけ失敗ログを次の修正試行へ返し、変更がない場合は停止する。reusable Actions workflow は verification job を fail-closed にする
