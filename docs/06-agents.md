...
2. 次のパスは変更しない: {protectedPaths + forbiddenPaths}
   (どうしても必要なら、変更せずに最終報告でその旨を説明し status を "blocked" にする)
3. git push・PR 作成・gh コマンドの実行は禁止(反映は別システムが行う)
4. 修正後、`{commands.verify}` が通ることを自分でも確認する
5. 既存のコードスタイル・規約(CLAUDE.md / AGENTS.md があれば従う)を尊重する
6. 修正が完了したら、変更をコミットする。コミットメッセージ: `kaizen: <変更の要約> (#{number})`
7. テストで保護できる修正には、可能な範囲で回帰テストを追加する
