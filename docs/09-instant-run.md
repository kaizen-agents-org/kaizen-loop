...
- 非 TTY(AI からの実行)では確認プロンプトが出せないため、明示 opt-in 設定で直接コミット判定に到達した場合は `instant.unattendedMode` に従う(§5)。デフォルトは PR への切り替え。AI に main を直接動かさせたい場合は `verifier.enabled: false` と直接コミット用の `policy.mode` に加え、`--yes` または `instant.unattendedMode: direct` を明示する
...
- 明示 opt-in 設定で direct commit 判定に到達した場合は、`fix` と同じ確認プロンプト / `instant.unattendedMode` に従う
...
日中の main 直接 push は夜間と違い、**開発者のローカル作業(未 push のコミット、進行中のブランチ)と競合しうる**。通常は PR を作成し、明示 opt-in 設定で直接コミット判定に到達した場合だけ以下で制御する:
...
確認できない文脈(`--json`、パイプ実行、`kaizen watch`)では、[04-nightly-pipeline.md](./04-nightly-pipeline.md) §6 の安全ゲートを通過した明示 opt-in の直接コミット判定に限り、設定 `instant.unattendedMode` に従う:
...
| `direct` | 設定済みの直接コミット経路へ進む |
| `reject` | 反映せず失敗コメントを残す |

> デフォルトを `pr` にする理由: 即時実行の無人トリガー(AI・watch)は日中に発火するため、開発者が気づかないうちに main が動くことを避ける。標準の夜間バッチも PR-first であり、直接コミットは明示 opt-in の例外として扱う。
...
| verifier step / リスク判定・保護パス・検証必須 | ✅ 同一 | ✅ 同一(`decideReflection` を共用) |
