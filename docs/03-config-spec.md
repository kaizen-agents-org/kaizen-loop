...
- `guardian.enabled: true` の場合、PR 作成後に vendored `skills/pr-guardian/SKILL.md` を `guardian.command exec` で実行する。PR の mergeable 化、`gh run watch` による CI 監視、未解決の actionable review feedback 対応、レビューコメントへの返信は skill 側の責務。各 pass 後に Kaizen Loop 本体が未解決・非 outdated の review thread を確認し、残っていれば `guardian.maxAttempts` まで再実行する
