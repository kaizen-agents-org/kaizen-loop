...
safety:
  # external は execution authorization と verifier を必須化。自組織運用のみ dogfood を明示できる
  operationMode: external      # external | dogfood
  # workspace / worktree 作成前に必要な空き容量(MB)
  minFreeDiskMb: 1024
  # 対象 owner 配下の登録済み repository にある open 生成 PR がこの数に達したら自動 intake を止める
  wipLimit: 5
...
issues:
  ...
  executionAuthorization:
    ...
    minimumPermission: triage # triage | write | maintain | admin
  selection:
...
- `safety.wipLimit` は対象 owner 配下にある Kaizen 登録済み repository の open 生成 PR 数に対する自動 intake の WIP 上限。未登録 repository の PR は数えない。生成判定は Kaizen の branch prefix (`kaizen/` / `codex/` / `claude/`) または title prefix (`[WIP] kaizen:` / `[scout]` / `[monitor]` / `kaizen:`) に基づき、bot 作成というだけでは生成 PR とみなさない。固定ブランチを再利用する既知の sync PR はこの判定から除外する。上限以上なら新しい Issue は選択せず、run summary に skip reason を残す。`kaizen status --metrics` は repository / organization の現在値、上限到達有無、最古の生成 PR の滞留日数を同じ登録済み repository scope で表示する
...
- `guardian.enabled: true` の場合、PR 作成後に vendored `skills/pr-guardian/SKILL.md` を `guardian.command exec` で実行する。`guardian.mode: sync` は foreground run 内で実行し、`async` は `~/.kaizen/projects/<slug>/guardian/jobs/` に永続 job を enqueue して `kaizen guardian watch` / `kaizen guardian run <pr>` で再開する。`guardian watch` と scheduled run は、既知の sync branch・同一 repository head・default base・`<!-- kaizen-pr-guardian:managed -->` marker をすべて満たす ready PR も採用する。durable job は隔離 worktree で実行し、成功後も open PR を再観測するため、同一 head に遅着した review thread は job を再活性化する。bot review の head 証拠は paginated REST reviews API の `commit_id` で判定する。PR の mergeable 化、CI 監視、actionable feedback 対応、レビューコメントへの返信は skill 側の責務。approval 不足は branch protection が明示要求している場合だけ blocker として扱う
...
`queue.health.state` は、run 自体が失敗した場合は `blocked`、成功した run で backlog が空なら `idle`、処理可能または処理済みなら `healthy`。成功した run でも eligible issue がありながら処理件数が 0 の場合は `degraded` とし、理由を `warning` に記録する。backlog 全件が同じ gate で skip された最初の run は `degraded`、同じ状態が `report.starvationRuns` 回続くと `starved` になり、`warning` と開始時刻 `since` を記録する。`reasonCode` はそれぞれ `run_failed`、`eligible_not_processed`、`repeated_gate`、`empty_queue` を使い、旧 run のように未記録でも読み込み可能とする。fleet health は `blocked` > `starved` > `degraded` > `healthy` の優先順で集約する。プロセス自体に失敗がなければ `result: "success"` は維持する。
