...
| パス | `protectedPaths`(強制 PR)/ `forbiddenPaths`(変更即失敗) | CI・秘密情報・ループ自身の設定の無審査変更 |
...
| テストがもともと壊れている(エージェントのせいでない) | ベースライン検証: **修正前に一度 verify を実行**し、失敗するなら「環境/既存の問題」として実行を中止し通知(Phase 2 で導入。Phase 1 は失敗コメントで人間が判断) | 実行中止 + 通知 |
...
5. (`kaizen:direct` ラベルがある)または(変更量が上限以下)
6. push 直前の rebase 後にも検証が成功した

ひとつでも欠ければ PR(または失敗)。この条件は実装上 1 つの純関数 `decideReflection(diffStats, labels, config, verifyResult): 'direct' | 'pr'` に集約し、ユニットテストで全パターンを固定する。
...
1. **週 1〜2 回、`kaizen run --dry-run`** で選択・判定だけ確認
