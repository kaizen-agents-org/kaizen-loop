...
| テストがもともと壊れている(エージェントのせいでない) | ベースライン検証: **修正前に一度 verify を実行**し、失敗するなら「環境/既存の問題」としてエージェントを起動しない | Issue へ失敗コメント + 次の Issue へ |
...
| ワークスペース破損 | fetch / setup 失敗 | 実行中止 + 通知。`kaizen doctor --repair` で再クローン |
...
- Kaizen Loop はトークン・API キーを保存しない。認証はすべて `gh` / `claude` / `codex` の既存セッションに委譲
...
- 設定 `run.latestStartHour`(デフォルト: 7)— スケジュール時刻から大幅に遅れて起動した場合(この時刻を過ぎていたら)、実行をスキップして「スキップした」通知のみ出す(日中の開発と夜間メンテの同時進行を防ぐ)
