...
| WIP | `safety.wipLimit` の登録 repository 横断・生成 PR backpressure | 生成 PR が review 帯域を超えて蓄積すること |
...
| ワークスペース破損・権限不一致 | fetch / setup 失敗、または `doctor` の `0700` 検査 | 権限は `kaizen doctor --repair` で修復。壊れた Git checkout の再クローンは手動対応 |
