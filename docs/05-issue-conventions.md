...
| `kaizen:needs-human` | オーケストレータ | 自動修正を断念(試行上限・情報不足)。通常の blocked は**人間が対応し、解決したらこのラベルを外すと再び対象に戻る**。ただし、最新結果が `retryableExternal: true`、または旧形式コメントでも既知の外部障害証拠を含む場合は、ラベルが残っていても次回自動実行で再試行する |
