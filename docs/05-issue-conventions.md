...
| `kaizen:needs-human` | オーケストレータ | 自動修正を断念(試行上限・情報不足)。**人間が対応し、解決したらこのラベルを外すと再び対象に戻る** |
...
`InProgress --> Open` と `InProgress --> NeedsHuman` では Issue branch と実装 checkpoint を保持する。再選択後は同じ branch を新しい worktree に接続し、直近の失敗理由を builder-agent へ渡して途中から再開する。PR 作成後は durable guardian job が merge-ready になるまで担当する。
