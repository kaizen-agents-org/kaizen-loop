...
| ラベル | 意味 |
|---|---|
| `kaizen:now` | `kaizen watch` 常駐モード(→ [09-instant-run.md](./09-instant-run.md) §3.3)が検知して即時処理する。処理開始時にオーケストレータが剥がす |

### Goal 連携(オーケストレータが管理)

| ラベル | 意味 |
|---|---|
| `kaizen:goal` | `kaizen goal run` が作成した Goal-linked Issue。本文の `kaizen-goal` marker で Goal state と紐づく |
