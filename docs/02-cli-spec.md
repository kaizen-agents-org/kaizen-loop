...
2. **対話セットアップ**(`--yes` で全デフォルト採用):
   - builder-agent へ渡す希望バックエンド(生成時のデフォルト: claude)。このリポジトリのコミット済み `.kaizen/config.yml` は `execution.builder.primary.provider: codex` を設定するため、通常の Issue 処理は Codex を希望バックエンドとして渡す
...
4. **GitHub ラベル作成**(冪等): `kaizen`, 実行ゲート (`kaizen:authorized` はメンテナによる実行承認、`kaizen:ready` は `issues.selection.mode: opt-in` の queued 実行選択), `kaizen:P0/P1/P2`, `kaizen:direct`, `kaizen:pr-only`, `kaizen:in-progress`, primary disposition (`kaizen:needs-human`, `kaizen:retryable`, `kaizen:blocked`, `kaizen:upstream-first`, `kaizen:not-actionable`, `kaizen:attempts-exhausted`), `kaizen:goal`, `kaizen:roadmap`, `kaizen:agent:claude`, `kaizen:agent:codex`
