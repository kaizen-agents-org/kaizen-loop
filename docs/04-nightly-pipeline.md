...
- Issue branch が前回の失敗・中断から残っている場合は、その branch を新しい worktree に再接続して実装を再開する。`origin/<defaultBranch>` から作り直さない
- 実装フェーズは `~/.kaizen/projects/<slug>/implementations/issue-<N>.json` に保存する。状態は `implementing` / `verifying` / `publishing` / `guardian` / `blocked` / `failed` / `complete` で、branch、attempt、直近の失敗理由、作成済み PR を記録する
- builder / verifier / publish が失敗した時点で未コミットの変更があれば checkpoint commit を作る。次回の builder には保存した失敗理由を渡し、同じ変更の続きから修正させる
...
3. `forbiddenPaths` に該当する変更があれば → **即失敗**(Issue branch に checkpoint として隔離し、自動 publish はしない)
...
- 途中変更は Issue branch に checkpoint commit として保存する
