...
ただし、結果コメントが `retryableExternal: true` の blocked は外部実行環境の一時障害として扱う。`command_missing`、`auth_failed`、timeout、rate limit などが該当し、`kaizen:needs-human` が残っていても次回の自動選択候補に戻す。再試行可能な結果で終了した時点で同ラベルは解除する。仕様不明・承認不足など通常の blocked は従来どおり人間対応待ちとして除外する。
