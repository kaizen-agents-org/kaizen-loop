...
flowchart LR
    ...
    C --> D["✅ 反映<br/>既定: PR 作成<br/>明示 opt-in: 直接コミット"]
...
| 02:00–06:00 | メンテナンスエージェント | Issue を優先度順に最大 N 件処理。修正 → テスト → PR 作成(明示 opt-in 時のみ直接コミット)。各 Issue に結果をコメント |
...
- **G2**: 翌朝には前日に登録された問題の修正 PR、または明示 opt-in された低リスク変更の直接反映が確認できること
