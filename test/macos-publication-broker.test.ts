...
describe('publication broker source contract', () => {
  ...
  it('keeps credentials root-only and broker responses bounded', async () => {
    ...
    expect(installer).toContain('validate_existing_private_directory');
    const leafValidation = installer.indexOf('validate_existing_private_directory\n');
    const parentNormalization = installer.indexOf('chown root:wheel "$private_root"');
    ...
    expect(installer).toContain('chown root:"$runtime_gid" "$private_directory"');
    ...
    expect(parentAclValidation).toBeLessThan(leafValidation);
