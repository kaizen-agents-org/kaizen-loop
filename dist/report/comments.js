export function buildResultComment(options) {
...
export function countAttempts(comments) {
    return comments.filter((comment) => {
        ...
        if (!marker)
            return false;
        if (marker.outcome === 'infrastructure-failure')
            return false;
        if (marker.humanConfirmationRequired === true)
            return true;
        return !marker.retryableExternal && !hasRetryableExternalEvidence(comment.body);
    }).length;
}
export function hasRetryableExternalBlock(comments) {
    ...
    return Boolean(marker?.marker?.outcome === 'blocked' &&
...
export function countConsecutiveRetryableBlocks(comments) {
    let count = 0;
...
export function agentSummary(result) {
    if (result.status === 'blocked')
        return result.blockedReason || result.summary;
    return result.summary;
}
function parseKaizenMarker(body, kind) {
...
function isOpenPullRequest(markerPr, openPullRequests) {
...
function formatOutcome(options) {
    if (options.outcome === 'pr-created')
    ...
    if (options.outcome === 'blocked') {
        ...
        if (options.blockDisposition === 'human-input-required')
            return 'Blocked; needs human input';
        if (options.blockDisposition === 'attempts-exhausted')
            return 'Blocked; retry budget exhausted';
        return 'Blocked; automation cannot proceed';
    }
    if (options.outcome === 'skipped')
        return 'Skipped';
    if (options.outcome === 'infrastructure-failure')
        return 'Infrastructure failure; implementation checkpoint preserved';
    return 'Failed';
}
function hasRetryableExternalEvidence(body) {
    return [
