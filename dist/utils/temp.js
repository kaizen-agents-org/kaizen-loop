...
export function resolveKaizenTempDir(cwd, env = process.env) {
...
export async function ensureKaizenTempDir(cwd, env = process.env) {
...
export async function envWithKaizenTemp(env = process.env, cwd) {
...
function shortKaizenTempRoot() {
    if (process.platform === 'win32')
        return path.join(os.tmpdir(), 'kaizen-loop');
    const uid = typeof process.getuid === 'function' ? process.getuid() : 'user';
    return path.join('/tmp', `kaizen-loop-${uid}`);
}
function hashTempScope(scope) {
