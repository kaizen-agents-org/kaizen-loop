...
scheduler
    ...
    await updateRegistry((latest) => {
        ...
        if (!current)
            throw new KaizenError(`Unknown Kaizen project: ${resolved.slug}`);
...
scheduler
    ...
    if (!job)
        throw new KaizenError('--job is required', 2);
    ...
    if (!isRecord(jobConfig.run))
        jobConfig.run = defaultSchedulerRun(schedule);
...
scheduler
    ...
    await updateRegistry((latest) => {
        for (const slug of disabled) {
            if (latest.projects[slug])
                latest.projects[slug].enabled = false;
        }
    });
...
function parseTrigger(value) {
    if (value === undefined)
        return undefined;
    if (value === 'manual' || value === 'scheduled' || value === 'afternoon' || value === 'instant' || value === 'watch')
        return value;
...
function parseSchedulerScheduleOptions(options) {
    ...
    if (selected.length !== 1)
        throw new KaizenError('Specify exactly one of --daily, --times, --every-hours, or --every-minutes.', 2);
    if (options.daily !== undefined)
        return { type: 'daily', time: parseSchedule(options.daily) };
    if (options.times !== undefined) {
        const times = options.times.split(',').map((time) => parseSchedule(time.trim()));
        if (times.length === 0)
            throw new KaizenError('Invalid --times: no times provided', 2);
    ...
    if (options.everyHours !== undefined) {
        ...
        if (everyHours > 23)
            throw new KaizenError('Invalid every-hours: must be between 1 and 23', 2);
        return {
            type: 'interval',
            everyHours,
    ...
    if (options.everyMinutes === undefined)
        throw new KaizenError('Specify --every-minutes.', 2);
    const everyMinutes = parsePositiveInteger(options.everyMinutes, 'every-minutes');
    if (everyMinutes > 1439)
        throw new KaizenError('Invalid every-minutes: must be between 1 and 1439', 2);
...
function defaultSchedulerRun(schedule) {
...
function parseIssueNumbers(value) {
    ...
    if (typeof value !== 'string')
        throw new KaizenError(`Invalid issue list: ${String(value)}`, 2);
    const issueNumbers = value.split(',').map((item) => item.trim()).filter(Boolean).map((item) => parsePositiveInteger(item, 'issue'));
    if (issueNumbers.length === 0)
        throw new KaizenError('Invalid issue list: no issue numbers provided', 2);
...
function parseOptionalPositiveInteger(value, name) {
    if (value === undefined)
        return undefined;
    if (typeof value !== 'string')
        throw new KaizenError(`Invalid ${name}: ${String(value)}`, 2);
...
function queueForReport(value, now) {
    if (value === true || value === false)
        return value;
    return now ? true : undefined;
}
function parseIssueArguments(values) {
    if (!Array.isArray(values) || values.length === 0)
        throw new KaizenError('At least one issue number is required', 2);
...
async function promptDirectCommit(context) {
...
async function promptImprove(plan) {
    const rl = createInterface({ input: process.stdin, output: process.stderr });
    try {
...
function printImprovePlan(plan) {
    ...
    if (plan.skipped.length > 0) {
        ...
        for (const issue of plan.skipped)
            console.error(`- #${issue.number}: ${issue.reason}`);
    }
}
