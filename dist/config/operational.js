...
export async function loadOperationalConfig(project, options) {
    if (options.preferWorkspace) {
        try {
            ...
            if (options.requireWorkspace || !isMissingConfig(error))
                throw error;
        }
    }
    return {
...
export function configDrift(local, workspace, paths) {
