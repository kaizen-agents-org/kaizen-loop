...
const noteSectionPattern = (labelPattern) => `(?:${labelPattern}\\s*:|\\*\\*${labelPattern}\\s*:\\*\\*)`;
...
const discoveredIssueInputSchema = z
    ...
    for (const field of ['title', 'expected', 'evidence']) {
        if (!issue[field].trim()) {
...
function optionalTrimmedField(key, value) {
...
function hasStructuredNotes(notes, labels) {
    ...
    if (!labels.every((label) => (notes.match(new RegExp(`${NOTE_SECTION_PREFIX}${noteSectionPattern(label)}`, 'g'))?.length === 1 &&
        new RegExp(`${NOTE_SECTION_PREFIX}${noteSectionPattern(label)}${contentPattern}`).test(notes)))) {
        return false;
    }
    const verification = new RegExp(`${NOTE_SECTION_PREFIX}${noteSectionPattern('Verification')}\\s*([\\s\\S]*?)(?=${NOTE_SECTION_PREFIX}${sectionPattern}|$)`).exec(notes)?.[1].trim();
