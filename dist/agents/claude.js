...
export function parseAgentResult(raw, durationMs = 0) {
    const parsedTopLevel = parseMaybeJson(raw);
    const finalText = typeof parsedTopLevel === 'object' && parsedTopLevel && 'result' in parsedTopLevel
        ? String(parsedTopLevel.result)
        : raw;
