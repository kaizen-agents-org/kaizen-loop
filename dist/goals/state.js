...
const goalStateSchema = z
    ...
    .strict();
export function goalsDir(projectSlug) {
    return path.join(projectStateDir(projectSlug), 'goals');
}
export function goalDir(projectSlug, goalId) {
    return path.join(goalsDir(projectSlug), goalId);
}
export function goalPath(projectSlug, goalId) {
...
export async function listGoalStates(projectSlug) {
    ...
    const goals = await Promise.all(entries.map(async (entry) => {
        try {
...
export function touchGoal(goal, now = new Date()) {
    return { ...goal, updatedAt: now.toISOString() };
}
function goalId(title, now) {
