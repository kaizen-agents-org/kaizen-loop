...
import { applyIssueDisposition, dispositionForBlockedAgent, dispositionForIntake, humanRequestForIntake } from './disposition.js';
import { ensureHumanRequest } from './humanRequest.js';
import { RunLock } from './lock.js';
import { enqueueManagedPrGuardianJobs, enqueuePrGuardianJob, runPendingPrGuardianJobs, runPrGuardianSkill } from './prGuardian.js';
import { buildIssueIntakeComment, evaluateIssueIntake, hasIssueIntakeDecisionComment } from './issueIntake.js';
...
export async function runKaizen(options) {
    ...
    if (options.dryRun) {
        if (options.scheduled)
            await assertExistingWorkspacePrivate(resolved.project.workspacePath);
...
async function runKaizenWithPreparedWorkspace(options, resolved) {
    try {
        ...
        try {
            ...
            if (selection.selected.length > 0) {
                ...
                if (!baseline.ok) {
                    runFailed = true;
                    summary.skipped.push(...selection.selected.map((skippedIssue) => ({
                    ...
                    try {
                        for (const issue of selection.selected) {
                            ...
                            try {
                                ...
                                if (!(error instanceof CheckpointBranchMissingError) &&
                        ...
                        const issueResults = await Promise.allSettled(worktrees.map((worktree) => processIssueInWorktree({
...
async function assertExistingWorkspacePrivate(workspacePath) {
    try {
...
export async function preflightVerifier(options) {
    ...
    try {
        ...
        if (options.config.verifier.update.mode !== 'canonical-main' ||
        ...
        try {
            let refreshed;
            let refreshedFailure;
            let refreshedCommand;
            try {
            ...
            try {
                ...
                if (rolledBack.protocol !== 'structured' ||
...
async function pinnedVerifierExecutable(runtime, expectedPackageRoot) {
...
function verifierRuntimeFailure(runtime, expectedCommit, source) {
    if (runtime.protocol !== 'structured') {
...
async function writeVerifierRuntimeDiagnostic(runtimePath, runtime, expectedCommit, source, recovery) {
    ...
    for (const issue of options.selection.selected) {
        ...
        try {
            ...
            if (decision.authorized)
                ...
                skipped.push({ number: issue.number, reason: decision.reason });
        }
        catch (error) {
            skipped.push({
                number: issue.number,
...
function assertJobEnabled(config, jobName) {
    if (!jobName)
        return;
    const configuredJob = config.scheduler.jobs[jobName];
    if (!configuredJob)
        throw new ConfigError(`Unknown scheduler job: ${jobName}`);
    if (!configuredJob.enabled)
        throw new ConfigError(`Scheduler job is disabled: ${jobName}`);
}
function schedulerMaxIssues(job) {
...
async function loadConfigIfPresent(repoDir) {
    try {
        return await loadConfig(repoDir);
    }
    catch (error) {
        if (error instanceof ConfigError && error.message.includes('Missing Kaizen config:'))
            return undefined;
        throw error;
    }
}
async function reconcileMergedPullRequestIssues(options) {
    ...
    for (const issue of options.issues) {
        ...
        const resolutions = await Promise.all(prNumbers.map((prNumber) => options.github.getPullRequestResolution(prNumber).catch(() => undefined)));
        for (const pr of resolutions) {
            if (!pr)
                continue;
            if (pr.state !== 'MERGED' && !pr.mergedAt)
                continue;
            if (!pr.closingIssuesReferences.some((reference) => reference.number === issue.number))
                continue;
            defaultBranch ??= await options.github.getRepositoryDefaultBranch().catch(() => '');
            if (defaultBranch && pr.baseRefName && pr.baseRefName !== defaultBranch)
                continue;
            if (!options.dryRun) {
                try {
                    await options.github.closeIssue(issue.number, `Kaizen Loop reconciled this issue after merged PR ${pr.url} did not leave the issue closed automatically.`);
...
function hasPullRequestResultMarker(issue) {
    return (issue.comments ?? []).some((comment) => comment.body.includes('kaizen-loop:result') && comment.body.includes('pr-created'));
}
function uniqueIssueNumbers(issueNumbers) {
    return [...new Set(issueNumbers)];
}
function openPullRequestFetchLimit(configuredLimit) {
    return Math.max(configuredLimit + 1, OPEN_PULL_REQUEST_LIMIT_CHECK_FETCH_LIMIT);
}
function assertRunWithinDeadline(deadlineAt) {
    throwIfShutdownRequested();
    if (Date.now() > deadlineAt)
        throw new Error('Kaizen run timeout exceeded.');
}
function boundedTimeoutMs(configuredTimeoutMs, deadlineAt) {
...
async function applyOpenPullRequestLimit(options) {
    if (!options.automatic || options.selection.selected.length === 0)
    ...
    const countedOpenPullRequests = options.openPullRequests.filter((pullRequest) => !isSyncPullRequest(pullRequest) && !options.resumeBranches?.has(pullRequest.headRefName ?? ''));
    ...
    if (options.selection.selected.length <= remaining)
        return options.selection;
...
function skipNewIssuesForLimit(selection, resumableIssueNumbers, reason) {
...
async function processIssue(options) {
    ...
    try {
        ...
        let agentResult = resumeAtVerifier
            ? {
                status: 'fixed',
        ...
        for (let retry = 0; retry <= options.config.run.maxVerifyRetries; retry += 1) {
            ...
            if (!skipBuilder) {
                ...
                discoveredFollowups.push(...await fileDiscoveredIssues({
            ...
            if (verifierResult.status === 'error' || verifierResult.status === 'timeout') {
                return withDiscoveredFollowups(await finishVerifierInfrastructureFailure(options, attempts, verifierResult, started, verifyResults), discoveredFollowups);
            }
            if (retry >= options.config.run.maxVerifyRetries) {
                return withDiscoveredFollowups(await finishFailed(options, agent, attempts, verifierBlockedReason(verifierResult), started, verifyResults), discoveredFollowups);
            }
        ...
        if (finalDiff.forbiddenFiles.length > 0) {
            return withDiscoveredFollowups(await finishFailed(options, agent, attempts, `Forbidden paths changed: ${finalDiff.forbiddenFiles.join(', ')}`, started, verifyResults), discoveredFollowups);
        }
        await saveImplementationState(options.stateDir, {
            issue: options.issue.number,
            branch,
        ...
        if (decision.action === 'direct') {
            ...
            if (directChoice === 'reject') {
                return withDiscoveredFollowups(await finishFailed(options, agent, attempts, `Direct commit rejected: ${decision.reason}`, started, verifyResults), discoveredFollowups);
            }
            if (directChoice === 'pr') {
                const pr = await reflectPullRequest({
                    workspace,
...
async function handOffReadyCheckpointPullRequest(options, attempt, pullRequest, started) {
    const guardian = await runPrGuardianAfterPullRequest({
        issue: options.issue,
        config: options.config,
        project: options.project,
...
async function finishCheckpointPullRequestMismatch(options, attempt, checkpoint, pullRequest, started) {
...
async function selectAgent(config, runCommand) {
...
function harnessScratchPaths(config) {
    if (!config)
        return [];
    const resultDir = path.dirname(config.verifier.resultPath);
    return resultDir && resultDir !== '.' ? [resultDir] : [];
}
async function finishBlocked(options, agent, attempt, agentResult, started) {
    let disposition = dispositionForBlockedAgent(agentResult);
...
async function finishVerifierInfrastructureFailure(options, attempt, verifierResult, started, verifyResults) {
    const previousState = await loadImplementationState(options.stateDir, options.issue.number);
...
async function publishDraftCheckpoint(options, attempt, reason, verifyResults = []) {
    try {
        const workspace = new WorkspaceManager(options.runCommand, options.project.workspacePath);
        ...
        if (existing && (existing.state === 'OPEN' || existing.state === undefined)) {
            ...
            if (updateError)
                return { pr: { number: existing.number, url: existing.url }, error: updateError };
        }
...
function checkpointPullRequestUpdateError(pullRequest, branch) {
...
function buildDraftCheckpointBody(issue, branch, attempt, reason, verifyResults, diff) {
...
async function checkpointPartialChanges(options, issue) {
    try {
        const workspace = new WorkspaceManager(options.runCommand, options.project.workspacePath);
        const diff = await workspace.collectCheckpointDiffStats(options.config);
        if (diff.forbiddenFiles.length > 0) {
...
async function resolveDirectCommitChoice(options) {
    if (options.trigger !== 'instant' && options.trigger !== 'watch')
        return 'direct';
...
function buildPullRequestBody(issue, agentResult, verifyResults, diff, riskReason, verifierResult) {
...
function summarizeIssueBody(body) {
    const trimmed = body.trim();
    if (!trimmed)
        return '(issue has no body)';
...
function escapeClosingReferences(text) {
    return text.replace(/\b(close[sd]?|fix(?:e[sd])?|resolve[sd]?)\s+#(\d+)\b/gi, '$1 \\#$2');
}
function verifierPrBodyLines(verifierResult) {
    const lines = [
    ...
    if (verifierResult.reason)
        lines.push(`reason: ${verifierResult.reason}`);
...
function verifierStructuredEvidenceLines(result) {
    const lines = [];
    for (const finding of result.mustFix ?? [])
        lines.push(`must_fix: ${formatVerifierFinding(finding)}`);
    for (const finding of result.shouldFix ?? [])
        lines.push(`should_fix: ${formatVerifierFinding(finding)}`);
...
function formatVerifierFinding(finding) {
    const evidence = finding.evidence ? ` — evidence: ${finding.evidence}` : '';
    return `[${finding.source}] ${finding.message}${evidence}`;
}
function verifierNotesWithoutStructuredDuplicates(result) {
    const duplicateLines = new Set();
    if (result.risk !== undefined)
        duplicateLines.add(`risk=${result.risk}`);
    if (result.confidence !== undefined)
        duplicateLines.add(`confidence=${result.confidence}`);
    if (result.mustFix?.length)
        duplicateLines.add(`must_fix=${result.mustFix.map((finding) => finding.message).join('; ')}`);
    if (result.shouldFix?.length)
        duplicateLines.add(`should_fix=${result.shouldFix.map((finding) => finding.message).join('; ')}`);
    return result.notes
        .split('\n')
...
function formatVerifierEvidenceGrade(verifierResult) {
    if (verifierResult.evidenceGrade === 'reported')
        return 'reported (未実行の可能性あり)';
    if (verifierResult.evidenceGrade === 'executed')
        return 'executed';
    return 'unknown';
}
function verifierEvidenceStrength(verifierResult) {
    if (!verifierResult)
        return '- unverified: verifier was not run for this PR body.';
    if (verifierResult.evidenceGrade === 'executed') {
...
function buildRunAbortComment(runId, reason, verifyResults) {
    const verify = verifyResults.length
        ? verifyResults.map((result) => `- ${result.ok ? '[x]' : '[ ]'} \`${result.command}\``).join('\n')
...
async function preparePrFallback(workspace, branch) {
    const git = workspace.git();
...
async function reflectDirect(options) {
    ...
    if (failedVerify)
        throw new Error(`post-rebase verification failed: ${failedVerify.command}`);
...
function verifierBlockedReason(result) {
    const detail = result.reason || result.summary;
...
function appendVerifierStructuredEvidence(reason, result) {
    const evidence = verifierStructuredEvidenceLines(result);
    return evidence.length ? `${reason}\n${evidence.join('\n')}` : reason;
}
function withDiscoveredFollowups(summary, discoveredFollowups) {
    if (discoveredFollowups.length === 0)
        return summary;
    return {
        ...summary,
...
async function fileDiscoveredIssues(options) {
    ...
    if (filed.length === 0)
        return filed;
    try {
        await options.github.comment(options.sourceIssue.number, `## Kaizen discovered follow-up issue${filed.length === 1 ? '' : 's'}
...
function buildPrLinkageRequirement(repo, issueNumber) {
...
function resolveReportedDiscoveredIssueRepo(repo, fallbackRepo) {
    if (!repo?.trim())
        return fallbackRepo;
...
function inferRegisteredRepoFromIssueText(issue, registry, options) {
    const text = [
        issue.title,
        issue.body,
        issue.expected,
...
function findRegisteredPathMatch(text, projects) {
    return projects
...
function containsPathCandidate(text, candidatePath) {
    let start = 0;
    while (start < text.length) {
        ...
        if (isPathTextBoundary(before) && isPathTextBoundary(after))
            return true;
        start = index + candidatePath.length;
    }
    return false;
}
function isPathTextBoundary(char) {
    return char === undefined || char === '/' || char === '\\' || !/[A-Za-z0-9._-]/.test(char);
}
function projectPaths(localPath, workspacePath) {
...
function requiredLabelsForDiscoveredIssue(config) {
...
function labelsForDiscoveredIssue(issue, requiredLabels) {
    const labels = new Set(requiredLabels);
    const severity = issue.severity?.trim().toUpperCase();
    if (severity && /^P[0-2]$/.test(severity))
        labels.add(`kaizen:${severity}`);
    for (const label of issue.labels ?? []) {
        const trimmed = label.trim();
        if (trimmed === 'kaizen' || /^kaizen:P[0-2]$/i.test(trimmed))
            labels.add(trimmed.replace(/:p/i, ':P'));
    }
    return [...labels];
}
...
async function updateLastRun(slug, summary) {
    let previousQueue;
    try {
        previousQueue = JSON.parse(await fs.readFile(path.join(projectStateDir(slug), 'last-run.json'), 'utf8')).queue;
    }
    catch {
...
async function readPersistedRunSummaries(stateDir) {
    try {
...
async function ensureNotPaused(stateDir) {
    try {
