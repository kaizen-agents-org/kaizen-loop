...
import { runCommand, runTrustedGitHubCli } from '../utils/command.js';
...
export async function verifyActionsFix(options) {
    ...
    if (setup && !setup.ok)
        throw new Error(`Setup failed: ${setup.command}\n${setup.output}`);
    ...
    if (diff.changedFiles === 0)
        throw new Error('Provider produced no file changes.');
    if (diff.forbiddenFiles.length)
        throw new Error(`Patch changes forbidden paths: ${diff.forbiddenFiles.join(', ')}`);
...
export async function publishActionsFix(options) {
    const command = options.runCommand ?? runCommand;
    ...
    if (sha256(patch) !== artifact.patchSha256)
        throw new Error('Verified patch hash does not match the publish artifact.');
    const context = await loadActionsContext(options.cwd, artifact.issue.number, command);
    if (context.repo !== artifact.repo || context.issue.title !== artifact.issue.title)
        throw new Error('Issue metadata changed after verification.');
    await assertAuthorized(context.github, context.repo, context.issue, context.config);
    const git = new GitClient(command, options.cwd);
    const head = await git.revParse('HEAD');
    if (head !== artifact.baseSha)
        throw new Error(`Publish checkout ${head} does not match verified base ${artifact.baseSha}.`);
...
async function loadActionsContext(cwd, issueNumber, command) {
    ...
    const repoResult = await runTrustedGitHubCli(command, ['repo', 'view', '--json', 'nameWithOwner', '--jq', '.nameWithOwner'], { cwd });
...
async function assertAuthorized(github, repo, issue, config) {
    ...
    if (!eligible)
        throw new Error(`Missing Kaizen eligibility label: ${config.issues.label}`);
    ...
    if (!labelActive)
        throw new Error(`Missing execution authorization label: ${authorization.label}`);
...
function buildPullRequestBody(artifact) {
...
export function encodeProviderResult(provider, finalMessage) {
