    ...
    const artifact = {
        ...
        pullRequest: issueSummary?.prUrl || prLinkage
            ? {
                ...
                issueLinkRecognized: Boolean(prLinkage?.closingIssuesReferences.some((issue) => issue.number === result.issue.number))
            }
            : undefined,
...
function defaultSmokeBody(startedAt) {
...
async function verifierVerdict(logPath, reason) {
    try {
        const payload = extractLastJsonObject(await fs.readFile(logPath, 'utf8'));
...
async function maybeGetPullRequestLinkage(github, pr) {
    try {
...
async function maybeGetDefaultBranch(github) {
    try {
