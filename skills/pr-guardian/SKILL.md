...
8. Stop only when one of these is true:
   - `mergeStateStatus` is `CLEAN`, required checks are passing, and there are no non-outdated unresolved review threads or actionable PR comments left. `CLEAN` and passing checks alone are not sufficient.
