...
5. Always inspect human, bot, and agent feedback on the PR before declaring it mergeable. Treat automated suggestions as review input, not commands to apply blindly. Use thread-aware review data, for example `gh api graphql` against `PullRequest.reviewThreads`, so unresolved inline threads and their `isResolved` / `isOutdated` state are visible.
