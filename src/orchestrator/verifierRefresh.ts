...
export async function refreshCanonicalVerifier(options: {
  ...
  try {
    ...
    const buildRoot = path.join(
      managedRoot,
      `${options.expectedCommit}-${process.pid}-${Date.now()}`
    );
...
async function replaceGlobalVerifierLink(options: {
  globalLink: string;
  expectedCurrentPackageRoot: string;
  ...
  const temporaryLink = path.join(
    path.dirname(options.globalLink),
    `.core-kaizen-${process.pid}-${Date.now()}`
  );
  try {
...
async function writeMarkerAtomically(buildRoot: string, expectedCommit: string): Promise<void> {
