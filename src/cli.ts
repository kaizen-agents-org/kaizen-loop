...
function runnerLabel(config: import('./config/schema.js').KaizenConfig): string {
...
function print(value: unknown, json = false): void {
  ...
  if (typeof value === 'object' && value && 'diff' in value && value.diff && typeof value.diff === 'object') {
    const diff = value.diff as { added?: string[]; retained?: string[]; removed?: string[] };
    console.log(JSON.stringify(value, null, 2));
    if (diff.added?.length) console.error(`ADD: ${diff.added.join(', ')}`);
    if (diff.retained?.length) console.error(`KEEP: ${diff.retained.join(', ')}`);
    if (diff.removed?.length) console.error(`REMOVE (requires --replace-all): ${diff.removed.join(', ')}`);
    return;
  }
  console.log(JSON.stringify(value, null, 2));
}
