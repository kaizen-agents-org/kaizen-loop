 ...
 */
export function resolveProfilePath(profile: string, profilesDir: string): string {
  ...
  if (!/^[a-z0-9][a-z0-9-]*$/i.test(profile)) {
    throw new ConfigError(
      `Invalid profile name: ${profile}. Use letters, digits, and hyphens, or pass a path to a profile file.`
    );
  }
  return path.join(profilesDir, `${profile}.yml`);
}

export async function loadProfile(profile: string, profilesDir: string): Promise<ProfileOverlay> {
 ...
 */
export function applySafetyFloor(config: Record<string, unknown>): {
  ...
  const protectedPaths = Array.isArray(policy.protectedPaths)
    ? (policy.protectedPaths as unknown[]).filter((entry): entry is string => typeof entry === 'string')
    : [];
  ...
  const forbiddenPaths = Array.isArray(policy.forbiddenPaths)
    ? (policy.forbiddenPaths as unknown[]).filter((entry): entry is string => typeof entry === 'string')
    : [];
